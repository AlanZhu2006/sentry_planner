...
void USARTServiceInit(USARTInstance *_instance)
{
    if (_instance == NULL || _instance->usart_handle == NULL ||
        _instance->usart_handle->hdmarx == NULL)
    {
        LOGERROR("[bsp_usart] RX DMA is required for registered USART instances");
        return;
    }
...
{
    return (uint8_t)(_instance != NULL && _instance->usart_handle != NULL &&
