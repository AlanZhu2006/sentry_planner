...
static uint8_t spi_bus_ready[SPI_BUS_CNT] = {1}; // 0: busy, 1: idle

static uint8_t *SPIBusReadyFlag(SPI_HandleTypeDef *handle)
{
    if (handle != NULL && handle->Instance == SPI4)
        return &spi_bus_ready[0];
    return NULL;
}

static void SPISelect(SPIInstance *instance)
{
    while (*instance->cs_pin_state == 0U)
    {
    }
    *instance->cs_pin_state = 0U;
    HAL_GPIO_WritePin(instance->GPIOx, instance->cs_pin, GPIO_PIN_RESET);
    instance->CS_State = GPIO_PIN_RESET;
}

static void SPIRelease(SPIInstance *instance)
{
    HAL_GPIO_WritePin(instance->GPIOx, instance->cs_pin, GPIO_PIN_SET);
    instance->CS_State = GPIO_PIN_SET;
...
SPIInstance *SPIRegister(SPI_Init_Config_s *conf)
{
    ...
    uint8_t *ready_flag = SPIBusReadyFlag(conf->spi_handle);
    if (ready_flag == NULL)
    {
        LOGERROR("[bsp_spi] only the configured SPI4 bus is supported");
        return NULL;
    }
    ...
    instance->id = conf->id;
    instance->cs_pin_state = ready_flag;
...
void SPITransmit(SPIInstance *spi_ins, uint8_t *ptr_data, uint8_t len)
{
    ...
    {
    case SPI_DMA_MODE:
        status = HAL_SPI_Transmit_DMA(spi_ins->spi_handle, ptr_data, len);
        break;
    case SPI_IT_MODE:
        status = HAL_SPI_Transmit_IT(spi_ins->spi_handle, ptr_data, len);
        break;
    case SPI_BLOCK_MODE:
        status = HAL_SPI_Transmit(spi_ins->spi_handle, ptr_data, len, 1000); // 默认50ms超时
    ...
    if (status != HAL_OK && spi_ins->CS_State == GPIO_PIN_RESET)
    {
        SPIRelease(spi_ins);
        LOGERROR("[bsp_spi] SPI4 transmit start failed");
    }
}
...
void SPIRecv(SPIInstance *spi_ins, uint8_t *ptr_data, uint8_t len)
{
    ...
    {
    case SPI_DMA_MODE:
        status = HAL_SPI_Receive_DMA(spi_ins->spi_handle, ptr_data, len);
        break;
    case SPI_IT_MODE:
        status = HAL_SPI_Receive_IT(spi_ins->spi_handle, ptr_data, len);
        break;
    case SPI_BLOCK_MODE:
        status = HAL_SPI_Receive(spi_ins->spi_handle, ptr_data, len, 1000);
        // 阻塞模式不会调用回调函数,传输完成后直接拉高片选结束
        SPIRelease(spi_ins);
        break;
    default:
    ...
    {
        SPIRelease(spi_ins);
        LOGERROR("[bsp_spi] SPI4 receive start failed");
    }
}
...
void SPITransRecv(SPIInstance *spi_ins, uint8_t *ptr_data_rx, uint8_t *ptr_data_tx, uint8_t len)
{
    ...
    case SPI_IT_MODE:
        ...
        break;
    case SPI_BLOCK_MODE:
        status = HAL_SPI_TransmitReceive(spi_ins->spi_handle, ptr_data_tx, ptr_data_rx, len, 1000); // 默认50ms超时
        // 阻塞模式不会调用回调函数,传输完成后直接拉高片选结束
        SPIRelease(spi_ins);
        break;
    default:
    ...
    {
        SPIRelease(spi_ins);
        LOGERROR("[bsp_spi] SPI4 transfer start failed");
    }
}
...
void HAL_SPI_ErrorCallback(SPI_HandleTypeDef *hspi)
{
    for (size_t i = 0; i < idx; i++)
    {
        ...
        {
            SPIRelease(spi_instance[i]);
            LOGERROR("[bsp_spi] SPI4 transfer failed");
            return;
        }
    }
}
