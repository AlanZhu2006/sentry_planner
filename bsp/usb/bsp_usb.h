...
/* Submit one asynchronous CDC transfer and return USBD_OK/BUSY/FAIL. */
uint8_t USBTransmit(const uint8_t *buffer, uint16_t len);

/* Abort a CDC IN transfer that did not complete before its caller's timeout. */
uint8_t USBAbortTransmit(void);
