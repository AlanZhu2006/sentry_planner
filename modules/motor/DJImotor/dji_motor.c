...
static DJIMotorPreCurrentHook pre_current_hook = NULL;
...
void DJIMotorRegisterPreCurrentHook(DJIMotorPreCurrentHook hook)
{
    pre_current_hook = hook;
}

static void DJIMotorComputePreCurrentRef(DJIMotorInstance *motor)
{
    ...
    motor_controller->pre_current_ref = pid_ref;
}

static int16_t DJIMotorComposeCurrentCommand(DJIMotorInstance *motor)
{
    ...
    float pid_ref = motor_controller->pre_current_ref;
...
void DJIMotorControl()
{
    ...
    // 先为所有电机计算速度环结束后的电流参考,允许共享钩子在电流环前统一修改
    for (size_t i = 0; i < idx; ++i)
    {
        DJIMotorComputePreCurrentRef(dji_motor_instance[i]);
    }

    if (pre_current_hook != NULL)
    {
        pre_current_hook();
    }
    ...
    for (size_t i = 0; i < idx; ++i)
    {
        ...
        set = DJIMotorComposeCurrentCommand(motor);
