...
Planar spin is currently taken from `/robot_control.chassis_spin_vel`; `bt_comm_adapter` writes it to `/cmd_vel_chassis_bt.angular.z` instead of using Nav2's `angular.z`. See [README_COMMANDS.md §1](README_COMMANDS.md#1-完整数据链路与职责划分) for the complete data paths and component ownership.

## Documentation

The root README provides the project overview and shortest onboarding path. Protocol details, tuning notes, and troubleshooting procedures live in the focused guides below. These detailed guides are currently maintained in Chinese.

| Document | Scope |
|---|---|
| [README_COMMUNICATION.md](README_COMMUNICATION.md) | Communication architecture, `sentry_bridge`, PTYs, SP/SX/ST, serial frames, ROS 2 topics, and real-robot communication checks |
| [README_BEHAVIOR_TREE_FLOW.md](README_BEHAVIOR_TREE_FLOW.md) | Strategy trees, nodes and plugins, `RobotControl`, Nav2 integration, Groot2, and debugging scripts |
| [README_LIDAR.md](README_LIDAR.md) | Mid-360, FAST-LIO, SLAM, Nav2, Gazebo RMUL2026, tuning, and localization troubleshooting |
| [README_COMMANDS.md](README_COMMANDS.md) | End-to-end data flow, launch commands, environment variables, maps, mapping, and staged real-robot bring-up |

Command references:

- [communication command.txt](communication%20command.txt): copy-ready communication commands; use `README_COMMANDS.md` as the source of truth for behavior and parameters.
...
See [README_COMMANDS.md](README_COMMANDS.md) and each workspace README for full commands, dependencies, and common build errors.
...
`start_robot.sh` does not start `sentry_bridge` automatically. See [README_COMMANDS.md §4](README_COMMANDS.md#4-nav_wsstart_robotsh调试用环境变量详解) and [§12](README_COMMANDS.md#12-实机联调) for port selection, environment variables, and the complete checklist.

## Recommended Validation Path

Validate the stack in stages: **Gazebo RMUL2026 and Nav2 tuning → bench communication and behavior-tree checks → low-speed localization and return-to-home tests in a small area → full-field integration**. Start with [README_LIDAR.md §6.4](README_LIDAR.md#nyush-gazebo-sim2real) and use [mid360 command.txt](mid360%20command.txt) for step-by-step commands.
...
The project is distributed under the [MIT License](LICENSE). NYUSH-specific protocol and chassis changes—including the 19-byte radar frame, `ref_yaw`, and sentry swerve-drive integration—are defined by `README_COMMUNICATION.md` and the control repository implementation.
