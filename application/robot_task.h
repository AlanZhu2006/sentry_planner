...
#if !defined(ROBOT_TYPE_damiao) && \
    !(defined(ROBOT_TYPE_sentry_swerve) && SENTRY_STEER_KEYBOARD_TEST)
...
__attribute__((noreturn)) void StartINSTASK(void const *argument)
{
    ...
    for (;;)
    {
        ...
        // 视觉姿态发送数据统一由 RobotCMDTask 写入，避免多任务并发覆盖
        osDelay(1);
    }
}
...
__attribute__((noreturn)) void StartMONITORTASK(void const *argument)
{
    LOGINFO("[freeRTOS] Monitor Task Start");
    DashboardInit();
    for (;;)
    {
        DashboardTask();
